"""Domain-specific specialist agents for Trading Journal AI.
...
SPECIALISTS = [('Setup Classifier', 'labels strategy type, timeframe, and execution context'), ('Risk Coach', 'scores position sizing, stop discipline, and R-multiple quality'), ('Bias Detector', 'identifies revenge trading, FOMO, hesitation, and overconfidence'), ('Performance Analyst', 'summarizes win/loss clusters and behavioral drift'), ('Review Writer', 'generates weekly trader improvement plans')]
...
def run_specialist_agents(payload: Dict[str, Any]) -> List[Dict[str, Any]]:
    subject = str(payload.get("subject") or payload.get("scenario") or payload.get("input") or "five-trade journal shows over-sizing after loss streak")
    ...
    for idx, (name, role) in enumerate(SPECIALISTS, 1):
        ...
        finding = f"{name} reviewed trading analytics signal: {subject}. Risk pattern={severity} confidence={confidence}."
        evidence = [
            f"domain=trading analytics",
