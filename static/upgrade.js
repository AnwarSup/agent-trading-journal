/**
 * Agentic Trading Journal — dApp Upgrade Wiring
...
(function() {
  ...
  async function apiFetch(path, opts = {}) {
    try {
      ...
      console.error('[Agentic Trading Journal] API error:', e);
  ...
  window.dappExportJSON = function(data, filename) {
    ...
    a.href = url;
    a.download = filename || '07-trading-journal-export.json';
    a.click();
  ...
  window.dappExportCSV = function(rows, filename) {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = filename || '07-trading-journal-export.csv';
    a.click();
    URL.revokeObjectURL(url);
